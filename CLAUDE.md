...
Each earlier file must define its global before a later one uses it: `YTCheckMessages`, `YTCheckI18n`, `YTCheckStorage`, `YTParser`, `YTDomObserver`, then `content.js` orchestrates. `popup.html`, `options.html` and `welcome.html` re-include the same files via their own `<script>` tags, and the service worker loads `messages.js` → `i18n.js` → `storage.js` with `importScripts` — which is why it is a **classic** worker (no `"type": "module"` in the manifest): these files declare globals, and an ES-module worker couldn't share them. **If you add a new module, wire it into the manifest's content_scripts list (and any HTML page or the service worker's `importScripts` that needs it) in the right position.**
...
- **`chrome.storage.local` → one key per video, `video:<videoId>`** (since 1.9.0). A record has `{ videoId, liked, disliked, watchedByProgress, title, channel, thumbnail, url, updatedAt, viewedAt, viewed }`. **`viewed` is always derived**: `liked || disliked || watchedByProgress`. `saveVideo` recomputes it on every write — never set `viewed` directly. **`viewedAt` is stamped once**, when the record first becomes viewed, and deleted when it stops being viewed; `updatedAt` moves on every write, so only `viewedAt` can date "when was this viewed" — the popup's today/week/month counters and its recent-history list. Records written before 1.7.0 have no `viewedAt`; read it through `YTCheckStorage.getViewedAt(record)`, which falls back to `updatedAt`. Retention pruning deliberately stays on `updatedAt` (last activity).
  - **Never touch `chrome.storage.local` outside `storage.js`**, and never go back to one big map: with it every write rewrote — and `onChanged` broadcast to every tab — the whole history, and concurrent writes erased each other. `getVideo`/`saveVideo`/`deleteVideo` touch a single key; only `getAllVideos` (and what builds on it: `getViewedIds`, `getStats`, `clearVideos`, `pruneOldVideos`, `exportVideos`) reads everything. Content scripts load `viewedIds` once, then keep it in step from `onChanged` via `YTCheckStorage.getVideoChanges(changes)` instead of re-reading.
  - Read-modify-write ops go through `queueWrite`, so writes in one context run in order (a like and a watch-progress mark on the same video used to overwrite each other).
  - Versions before 1.9.0 kept a single `videos` map. `migrateLegacyVideos()` moves it into per-video keys (existing per-video records win) and removes it; every video op awaits it once per context, and the service worker triggers it on `onInstalled`/`onStartup`. The export/import JSON still uses the `{ videos: { videoId: record } }` shape, so old backups keep importing.
...
1. **Listing pages** (Home, Search, sidebar, etc.): `YTDomObserver.start` runs a debounced `MutationObserver` matching `YTParser.VIDEO_ELEMENTS_SELECTOR`; matched cards get a badge if their `videoId` is in the viewed set. A floating, draggable **page counter** tallies viewed/total. When the page carries a playlist queue (`YTParser.PLAYLIST_PANEL_SELECTOR` matches — `/watch?list=…`, autoplay queue, Mix), the counter scopes itself to those items only and switches its label to `viewedInPlaylist`, so it reads as playlist progress instead of lumping the queue in with the recommendations beside it. Cards linking with `list=LL` — the user's "Liked videos" list, on `/playlist?list=LL` or in its queue (`YTParser.isLikedListItem`) — are also written as liked by `syncLikedListItems`: that list is YouTube's own record of Likes given on any device, the only way the extension learns about a Like from the phone or TV.
2. **Watch page** (`/watch`): reads the like/dislike button state (retry loop + attribute observer) and persists it; **watch-progress tracking** listens to the `<video>` `timeupdate` and marks viewed past a threshold (on by default, `settings.trackWatchProgress`). Injects the indicator pill — "you already rated this" for a like/dislike, "you already watched this" for a watch-progress-only record (`WATCH_INDICATOR_KINDS`; a rating wins when both apply). Shorts use the same indicator.
...
`src/background/service-worker.js`: on install, opens `src/onboarding/welcome.html` and injects the content scripts into open YouTube tabs; on install/update/startup it runs the storage migration and arms a daily `chrome.alarms` job that prunes videos older than `settings.historyRetentionDays` (0 = keep forever) through `YTCheckStorage.pruneOldVideos`. It routes no messages: the popup and options page message content scripts directly (`chrome.tabs.sendMessage` — `refresh`, `clearHistory`, `settingsChanged`), and the worker only sends `ping` to detect a live content script.
